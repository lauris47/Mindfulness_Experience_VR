...
using UnityEngine;
    ...
    public class PlanarReflection : MonoBehaviour
    {
        ...
        [Range(0.1f,1.0f)]
        public float textureMultiplier = 0.25f;
        Vector3 m_Oldpos;
        ...
        {
            RenderTexture rt = new RenderTexture(Mathf.FloorToInt(cam.pixelWidth*textureMultiplier),
                Mathf.FloorToInt(cam.pixelHeight*textureMultiplier), 24);
            if(antiAlias == AntiAliasing.None)
            {
                rt.antiAliasing = 0;
            }
            else if(antiAlias == AntiAliasing.Two_Samples)
            {
                rt.antiAliasing = 2;
            }
            else if (antiAlias == AntiAliasing.Four_Samples)
            {
                rt.antiAliasing = 4;
            }
            else
